...
namespace Lean.Touch
{
	...
	public class LeanSelectableRendererColor : LeanSelectableBehaviour
	{
		...
		{
			if (cachedRenderer == null) cachedRenderer = GetComponentsInChildren<Renderer>();

			if (AutoGetDefaultColor == true)
			{
				for (int j = 0; j < cachedRenderer.Length; j++)
				{
					var material0 = cachedRenderer[j].sharedMaterial;
			...
			{
				for (var i = cachedRenderer.Length - 1; i >= 0; i--)
                {
					cachedRenderer[i].sharedMaterials = cachedRenderer[i].materials;
        ...
        {
			DefaultColor = color;
            for (int i = 0; i < cachedRenderer.Length; i++)
            {
				cachedRenderer[i].material.color = color;
			}
        }
		...
		private void ChangeColor(Color color)
		{
            ...
            for (int j = 0; j < cachedRenderer.Length; j++)
            {
				var materials = cachedRenderer[j].sharedMaterials;
